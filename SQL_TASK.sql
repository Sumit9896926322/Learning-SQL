...
/*its not allowed to update table without the use of primary key by default*/ 
 update albums set release_year = 1986 where release_year is null;
 
insert into bands (name) values('Desi crew'),('musical doctorz');
insert into albums (name,release_year,band_id) values('shada',2018,8),('jaguar',2019,9);

/*Order of deletion here is albums > bands*/

delete from albums where id = 19;
delete from bands where id = 9;
select * from bands;
select * from albums;
 ...
 from albums 
 join songs where albums.id = songs.album_id group by albums.name;
 
 
 /*Extract all songs from band,by including two join */
 
 select 
		bands.name as 'Band',
		count(albums.id) as 'Number of Songs'
 from bands 
 join albums on bands.id = albums.band_id 
